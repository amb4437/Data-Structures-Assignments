...
public class Graph {
	...
	/*Code partially copied from class notes */
	public int astar( Vertex start, Vertex goal, Heuristic h ) {
		...
		while (!fringe.isEmpty()){
			...
			if (goal.equals(v))
				return removeCount;  //Should this be break?

			if (!v.visited){
				v.visited = true;

				for ( Edge e : v.edges ){
					int cost = v.cost + e.cost;
					if (cost < e.target.cost){
						e.target.cost = cost;
	...
	public static final Heuristic dist = new Heuristic() {
		public Integer fn(Vertex from, Vertex to) {
			...
			public static final Heuristic randombs = new Heuristic() {
				public Integer fn(Vertex from, Vertex to) {
					return from.distanceTo(to) * ((int) rng.nextDouble()); 
				}
			};
			...
			public static final Heuristic randomlies = new Heuristic() {
				public Integer fn(Vertex from, Vertex to) {
					...
					public Cons pathto(String city) {
						...
						Vertex vertex = vertex(city);
						Cons path = Cons.list(vertex.contents);

						while (vertex !=null){
							path = Cons.cons(vertex.contents, path);
