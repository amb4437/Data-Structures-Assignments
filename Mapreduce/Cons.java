    ...
    // ****** your code starts here ******
     ...
     // 2. letter count, case-insensitive
            ...
            public void map(String key, String value, MapReduce mr) {
            	...
            	for (int i = 0; i < value.length(); i++){
            		...
            		if (Character.isLetter(character))
            			mr.collect_map(Character.toString(character), list("1"));
            	}
        ...
        final Reducer red2 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
            	int sum = 0;
            	Object current;
            	for (Cons first = value; first != null; first = rest(first)){
            		current = first(first);
            		current = first((Cons) current);
            		
            		sum += Integer.decode((String) current);
            	}
     ...
     // 3. word count, case-insensitive
        ...
        final Mapper map3 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
            	value = value.toLowerCase();
            	String[] foundWords = value.split("[^a-z]");
            	
            	for (String currentWord: foundWords){
            		if (currentWord.length() > 0)
            			mr.collect_map(currentWord, list("1"));
            	}
        ...
        final Mapper map4 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
                ...
                while ((current = value.indexOf(grepkey, current)) != -1){
        ...
        final Reducer red4 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
            	...
            	for (Cons first = value; first != null; first = rest(first)){
            		...
            		sum = Integer.decode((String) current);	
            		reduce = cons(sum, reduce);
            	}
            	mr.collect_reduce(key, nreverse(reduce));
        ...
        final Mapper map5 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
            	...
            	for (String currentWord: foundWords){
            		if (currentWord.length() > 0)
            			mr.collect_map(currentWord, list(key));
            	}
     ...
     // 6. movie reviews
        ...
        final Mapper map6 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
            	...
            	mr.collect_map(nums[0], list(nums[2]));
        ...
        final Reducer red6 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
            	...
            	mr.collect_reduce(key, sum/valLength);
